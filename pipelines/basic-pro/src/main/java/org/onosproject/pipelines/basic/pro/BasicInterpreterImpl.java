...
public class BasicInterpreterImpl extends AbstractHandlerBehaviour
    ...
    private static final Map<Integer, PiTableId> TABLE_MAP =
            new ImmutableMap.Builder<Integer, PiTableId>()
                    .put(0, BasicConstants.INGRESS_ACL_TABLE)
                    .build();
    ...
    public PiAction mapTreatment(TrafficTreatment treatment, PiTableId piTableId)
        ...
        if (treatment.allInstructions().isEmpty()) {
            ...
            return PiAction.builder().withId(INGRESS_ACL_DROP).build();
        ...
        switch (instruction.type()) {
            case OUTPUT:
                if (piTableId.equals(INGRESS_ACL_TABLE)) {
                    return outputPiAction((OutputInstruction) instruction, INGRESS_ACL_SET_EGRESS_PORT);
                } else {
    ...
    private PiAction outputPiAction(OutputInstruction outInstruction, PiActionId piActionId)
        ...
        if (!port.isLogical()) {
            ...
            return PiAction.builder().withId(INGRESS_ACL_SEND_TO_CPU).build();
        } else {
            throw new PiInterpreterException(format(

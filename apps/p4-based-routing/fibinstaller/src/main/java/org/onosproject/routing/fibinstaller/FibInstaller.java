...
package org.onosproject.p4basedrouting.fibinstaller;
...
import static org.onosproject.p4basedrouting.fibinstaller.OsgiPropertyConstants.ROUTE_TO_NEXT_HOP;
import static org.onosproject.p4basedrouting.fibinstaller.OsgiPropertyConstants.ROUTE_TO_NEXT_HOP_DEFAULT;

/**
 * Programs routes to a single OpenFlow switch.
 */
@Component(immediate = true, property = { ROUTE_TO_NEXT_HOP + ":Boolean=  " + ROUTE_TO_NEXT_HOP_DEFAULT })
public class FibInstaller {
    ...
    private static final String P4_TABLE_NAME = "SwitchIngress.unicast.ipv4_fib.rib";
    ...
    private ConfigFactory<ApplicationId, McastConfig> mcastConfigFactory =
            ...
            SubjectFactories.APP_SUBJECT_FACTORY, McastConfig.class, "multicast") {
        @Override
    ...
    private void processRouterConfig() {
        Set<RoutersConfig.Router> routerConfigs = RoutingConfiguration.getRouterConfigurations(networkConfigService,
                routerAppId);
         ...
         *
         * RouteEvent routeEvent = new RouteEvent(RouteEvent.Type.ROUTE_ADDED,
         * resolvedRoute); routeListener.event(routeEvent);
         */
    ...
    private void cleanUp() {
        ...
        // clean up the routes.
        prefixToNextHop.entrySet().stream().map(e -> new Route(Route.Source.UNDEFINED, e.getKey(), e.getValue()))
    ...
    private Router createRouter(RouterInfo info) {
        return new Router(info, interfaceService, deviceService, this::provisionInterface, this::unprovisionInterface,
                false);
    }
    ...
    private void updateRoute(ResolvedRoute route) {
        ...
        try {
            ...
            flowObjectiveService.forward(deviceId, generateRibForwardingObj(route.prefix(), nextId).add());
            log.trace("Sending forwarding objective {} -> nextId:{}", route, nextId);
        } catch (NullPointerException e) {
            log.error("Catch The exception: {}", ExceptionUtils.getFullStackTrace(e));
            log.error("Wait for fix");
        }
    }
    ...
    private void deleteRoute(Route route) {
        ...
        /*
         * Group group = deleteNextHop(route.prefix()); if (group == null) {
         * log.warn("Group not found when deleting {}", route); return; }
         */

        flowObjectiveService.forward(deviceId, generateRibForwardingObj(route.prefix(), null).remove());
    }
    ...
    private ForwardingObjective.Builder generateRibForwardingObj(IpPrefix prefix, Integer nextId) {
        ...
        ForwardingObjective.Builder fwdBuilder = DefaultForwardingObjective.builder().fromApp(fibAppId).makePermanent()
                .withSelector(selector).withPriority(priority).withFlag(ForwardingObjective.Flag.SPECIFIC);
    ...
    private synchronized void addNextHop(ResolvedRoute route) {
        ...
        if (nextHopsCount.count(route.nextHop()) == 0) {
            ...
            TrafficTreatment.Builder treatment = DefaultTrafficTreatment.builder().setEthSrc(egressIntf.mac())
            ...
            NextObjective.Builder nextBuilder = DefaultNextObjective.builder().withId(nextId)
                    .addTreatment(treatment.build()).withType(NextObjective.Type.SIMPLE).fromApp(fibAppId);
            if (metabuilder != null) {
    ...
    private void updateFilteringObjective(InterfaceProvisionRequest routerIntf, boolean install) {
        ...
        fob.withKey(Criteria.matchInPort(intf.connectPoint().port())).addCondition(Criteria.matchEthDst(intf.mac()))
    ...
    private void sendFilteringObjective(boolean install, FilteringObjective.Builder fob, Interface intf) {
        ...
        ObjectiveContext context = new DefaultObjectiveContext(
                ...
                (objective, error) -> log.error("Failed to install filter for interface {}: {}", intf, error));
    ...
    private VlanId ingressVlan() {
        McastConfig mcastConfig = networkConfigService.getConfig(coreAppId, McastConfig.class);
    ...
    private void generateFlow(DeviceId deviceId, PortNumber portNumber, IpPrefix prefix) {
        TrafficSelector.Builder selectorBuilder = DefaultTrafficSelector.builder();
        TrafficTreatment.Builder treatment = DefaultTrafficTreatment.builder();
        FlowEntry.Builder flowEntry = DefaultFlowEntry.builder();
        selectorBuilder.matchIPDst(prefix);
        treatment.setOutput(portNumber);
        flowEntry.forDevice(deviceId);

        flowEntry.forTable(PiTableId.of(P4_TABLE_NAME));
        flowEntry.withPriority(41000);
        flowEntry.withSelector(selectorBuilder.build());
        flowEntry.withTreatment(treatment.build());
        flowEntry.fromApp(fibAppId);
    ...
    private class InternalRouteListener implements RouteListener {
        ...
        public void event(RouteEvent event) {
            ...
            case ROUTE_ADDED:
            case ROUTE_UPDATED:
                IpAddress nextHopIpAddress = event.subject().nextHop();
                Set<Host> hostSet = hostService.getHostsByIp(nextHopIpAddress);
                for (Host host : hostSet) {
                    generateFlow(host.location().deviceId(), host.location().port(), event.subject().prefix());
                }
                updateRoute(route);
                break;
            case ROUTE_REMOVED:
